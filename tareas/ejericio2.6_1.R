...
######################################################################################
###     a) Ajustar un modelo de regresión lineal múltiple que relacione la cantidad de 
###     juegos ganados con las yardas por aire del equipo (x2)
###     , el porcentaje de jugadas por tierra (x7) 
###     y las yardas por tierra del contrario (x8).
######################################################################################
...
plot(M1$model$x2, residuales,
     xlab = "x2",
     ylab = "Residuales",
     main = "Residuales vs x2")
abline(h = 0, col = "red")

plot(M1$model$x7, residuales,
     xlab = "x7",
     ylab = "Residuales",
     main = "Residuales vs x7")
abline(h = 0, col = "red")

plot(M1$model$x8, residuales,
     xlab = "x8",
     ylab = "Residuales",
...
###################################################################################
#     h)  un intervalo de confianza de 95% para la cantidad media de juegos ganados 
#         por un equipo cuando x2=2300, x7=56 y x8=2100.
###################################################################################
...
#################################
#     i)  Ajustar un modelo a esos datos, usando solo x7 y x8 como regresores y 
...
R2
R2ADJ
##### claramente estos nuevos r son mas altos y mas parecidos entre ellos asi que son mejores
...
##############################################################################
#     k)  También, un intervalo de confianza de  
#         95% para la cantidad media de juegos ganados por un equipo cuando x7 =  56 y x8=2100
#         Comparar las longitudes de esos intervalos de confianza con las longitudes de los correspondientes al modelo anterior.
###########################################

x0_M2   <- c(1, 56, 2100)
y0_hat2 <- as.numeric(t(x0_M2) %*% beta2)
se_y0_2 <- sqrt(varest2 * (t(x0_M2) %*% XtX_inv2 %*% x0_M2))
IC_media_M2 <- c(LI = y0_hat2 - tcrit2*se_y0_2,
                 LS = y0_hat2 + tcrit2*se_y0_2)
y0_hat2
IC_media_M2
...
#################################
#     l)  ¿Qué conclusiones se pueden sacar de este problema, 
#         acerca de las consecuencias de omitir un regresor importante de un modelo?

#################################

#el modelo completo da una estimacion mayor al reducido 
#como vimos desde el inicio sabiamos que era una variable significante y haberla quitado claramente 
#a reducido su desempeño